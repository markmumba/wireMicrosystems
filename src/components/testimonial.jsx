

function Testimonial() {

  return (
    <div className="bg-white flex flex-col pl-16 pr-14 pt-12 pb-8 items-start max-md:px-5">
      <div className="text-black text-5xl  font-bold leading-[58px] w-[560px] max-w-full mt-16 max-md:text-4xl max-md:leading-[54px] max-md:mt-10">
        Satisfied Customers
      </div>
      <div className="text-black text-lg leading-7 mt-6 max-md:max-w-full">
        Hear what our customers have to say about our products and cost savings.
      </div>
      <div className="justify-center self-stretch mt-20 max-md:max-w-full max-md:mt-10">
        <div className="gap-5 flex md:px-20 max-md:flex-col max-md:items-stretch max-md:gap-0">
          <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
            <div className="items-stretch flex grow flex-col max-md:max-w-full max-md:mt-10">
              <div className="items-stretch flex w-[116px] max-w-full gap-1 self-start max-md:justify-center">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
              </div>
              <div className="text-black text-xl font-bold leading-7 mt-8 max-md:max-w-full">
                Our products are top-notch and have helped us save a significant
                amount of money.
              </div>
              <div className="items-start flex gap-5 mt-8 self-start max-md:justify-center">
                <img
                  loading="lazy"
                  srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-square object-contain object-center w-14 overflow-hidden self-center shrink-0 max-w-full my-auto rounded-[50%]"
                />
                <div className="items-stretch self-center flex grow basis-[0%] flex-col my-auto">
                  <div className="text-black text-base font-semibold leading-6 whitespace-nowrap">
                    John Doe
                  </div>
                  <div className="text-black text-base leading-6 whitespace-nowrap">
                    CEO, XYZ Company
                  </div>
                </div>
                <div className="bg-black self-stretch w-px shrink-0 h-[61px]" />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/068770d480626fb16b61a4c9daf02e9a0015cca5d8ebfbbae9bd57eb331a73cb?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[2.5] object-contain object-center w-[120px] overflow-hidden self-center shrink-0 max-w-full my-auto"
                />
              </div>
            </div>
          </div>
          <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
            <div className="items-stretch flex grow flex-col max-md:max-w-full max-md:mt-10">
              <div className="items-stretch flex w-[116px] max-w-full gap-1 self-start max-md:justify-center">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
              </div>
              <div className="text-black text-xl font-bold leading-7 mt-8 max-md:max-w-full">
                I highly recommend their products. They are reliable and
                cost-effective.
              </div>
              <div className="items-start flex gap-5 mt-8 self-start max-md:justify-center">
                <img
                  loading="lazy"
                  srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/e31c4459d8759fc74a4f36780203658e0c99a42b36ddb116f5196db95ef3aee1?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-square object-contain object-center w-14 overflow-hidden self-center shrink-0 max-w-full my-auto rounded-[50%]"
                />
                <div className="items-stretch self-center flex grow basis-[0%] flex-col my-auto">
                  <div className="text-black text-base font-semibold leading-6 whitespace-nowrap">
                    Jane Smith
                  </div>
                  <div className="text-black text-base leading-6 whitespace-nowrap">
                    CTO, ABC Corporation
                  </div>
                </div>
                <div className="bg-black self-stretch w-px shrink-0 h-[61px]" />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/068770d480626fb16b61a4c9daf02e9a0015cca5d8ebfbbae9bd57eb331a73cb?apiKey=f0c7f3edffe4460ca2a52c85cd1fec66&"
                  className="aspect-[2.5] object-contain object-center w-[120px] overflow-hidden self-center shrink-0 max-w-full my-auto"
                />
              </div>
            </div>
          </div>
        </div>
      </div>

    </div>
  );
}

export default Testimonial;

